...
psql $DBNAME -c "create table t (a bigint) with (fillfactor = $fillfactor)";
psql $DBNAME -c "insert into t select $maxvalue * random() from generate_series(1,$nrows) s(i)";
psql $DBNAME -c "create index on t using brin (a int8_minmax_multi_ops) with (pages_per_range = $rangesize)";

psql $DBNAME -t -A -c "select a from t order by random() limit $nqueries" > int8.txt 2>&1;
