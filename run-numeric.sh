...
psql $DBNAME -c "create table t (a numeric) with (fillfactor = $fillfactor)";
psql $DBNAME -c "insert into t select $maxvalue * random() from generate_series(1,$nrows) s(i)";
psql $DBNAME -c "create index on t using brin (a numeric_minmax_multi_ops) with (pages_per_range = $rangesize)";

psql $DBNAME -t -A -c "select a from t order by random() limit $nqueries" > numeric.txt 2>&1;
...
for i in `seq 1 $nqueries`; do
    ...
    v=`psql -t -A $DBNAME -c "select ($maxvalue * random())"`
