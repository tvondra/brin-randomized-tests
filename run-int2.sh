...
maxvalue=`psql $DBNAME -t -A -c "select (1 + random() * 32000)::int"`

suffix=$DBNAME

echo "maxvalue $maxvalue"

psql $DBNAME -c "create table t (a smallint) with (fillfactor = $fillfactor)";
psql $DBNAME -c "insert into t select $maxvalue * random() from generate_series(1,$nrows) s(i)";
psql $DBNAME -c "create index on t using brin (a int2_minmax_multi_ops) with (pages_per_range = $rangesize)";

psql $DBNAME -t -A -c "select a from t order by random() limit $nqueries" > int2.txt 2>&1;
