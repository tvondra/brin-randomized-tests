...
psql $DBNAME -c "create table t (a float4) with (fillfactor = $fillfactor)";
psql $DBNAME -c "insert into t select $maxvalue * random() from generate_series(1,$nrows) s(i)";
psql $DBNAME -c "create index on t using brin (a float4_minmax_multi_ops) with (pages_per_range = $rangesize)";

psql $DBNAME -t -A -c "select a::float8 from t order by random() limit $nqueries" > float4.txt 2>&1;
...
set enable_indexscan = off;
set max_parallel_workers_per_gather = 0;
explain select count(*) from t where a = ($v - 1);
select count(*) from t where a = ($v - 1);
EOF
...
for i in `seq 1 $nqueries`; do
    ...
    v=`psql -t -A $DBNAME -c "select ($maxvalue * random())::float4::float8"`
